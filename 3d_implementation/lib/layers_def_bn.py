...
#     return relu
def conv_bn_relu(inputs, output_channels, kernel_size, stride, is_training, keep_prob, name, padding='same', use_bias=False, bn_scope=''):
    print name+'/'+bn_scope
    with tf.variable_scope(name):
        conv = conv3d(inputs, output_channels, kernel_size, stride, padding=padding, use_bias=use_bias, name=name+'_conv')
        bn = batch_norm(conv, is_training = is_training, scope = name+'/'+bn_scope, trainable =True)
        relu = tf.nn.leaky_relu(features=bn, name=name+'_relu')
        # dropped = tf.nn.dropout(relu, keep_prob)
...
# 3D Deconvolution
def deconv3d(inputs, output_channels, kernel_size, name='deconv'):
    ...
    dev_filter = tf.get_variable(
        ...
        shape=[kernel_size[0], kernel_size[1], kernel_size[2], output_channels, in_channels],
...
#         # dropped = tf.nn.dropout(relu, keep_prob)
#     return relu
def deconv_bn_relu(inputs, output_channels, is_training, kernel_size, keep_prob, name, bn_scope=''):
    with tf.variable_scope(name):
        deconv = deconv3d(inputs=inputs, output_channels=output_channels, kernel_size=kernel_size, name=name+'_deconv')
        bn = batch_norm(deconv, is_training=is_training, scope=name+'/'+bn_scope)
        relu = tf.nn.leaky_relu(features=bn, name=name+'_lrelu')
        # dropped = tf.nn.dropout(relu, keep_prob)
    return relu
